...
import com.base.common.system.util.BaseDataAutorUtils;
...
public class JwtUtil {
     ...
     * @param request
     * @return
     * @throws BaseBootException
     */
    public static String getUserNameByToken(HttpServletRequest request) throws BaseBootException {
        String accessToken = request.getHeader("X-Access-Token");
        String username = getUsername(accessToken);
        if (oConvertUtils.isEmpty(username)) {
            throw new BaseBootException("未获取到用户");
        }
        return username;
    }
...
//		System.err.println("accessToken------------------------------"+accessToken);
        if (com.base.common.util.oConvertUtils.isEmpty(accessToken)) {
            return null;
        }
        String username = getUsername(accessToken);
        if (com.base.common.util.oConvertUtils.isEmpty(username)) {
            throw new JeecgBootException("未获取到用户");
        }
        RedisUtil redisUtil = SpringContextUtils.getApplicationContext().getBean(RedisUtil.class);
        LoginUser sysUser = JSONObject.parseObject(redisUtil.get(CacheConstant.SYS_USERS_CACHE_JWT + ":" + accessToken).toString(),LoginUser.class);
        log.info("redis==用户信息==={}",sysUser);
