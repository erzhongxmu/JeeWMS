...
public class QueryGenerator {
	 ...
	 * <br>错误示例:如QueryWrapper<BaseDemo> queryWrapper = new QueryWrapper<JBaseDemo>(BaseDemo);
	 * <br>正确示例:QueryWrapper<BaseDemo> queryWrapper = new QueryWrapper<BaseDemo>();
...
//					 add -end 添加判断为字符串时设为全模糊查询
	...
	public static Map<String, SysPermissionDataRuleModel> getRuleMap(List<SysPermissionDataRuleModel> list) {
		Map<String, SysPermissionDataRuleModel> ruleMap = new HashMap<String, SysPermissionDataRuleModel>();
		if(list==null){
			list = BaseDataAutorUtils.loadDataSearchConditon();
		}
		if(list != null&&list.size()>0){
			if(list.get(0)==null){
				return ruleMap;
