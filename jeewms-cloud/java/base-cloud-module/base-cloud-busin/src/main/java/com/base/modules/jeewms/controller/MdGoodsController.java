...
@Api(tags="商品信息")
@RestController
@RequestMapping("/jeewms/mdGoods")
@Slf4j
public class MdGoodsController extends JeecgController<MdGoods, IMdGoodsService> {
	@Autowired
...
//		QueryWrapper<MdGoods> queryWrapper = QueryGenerator.initQueryWrapper(mdGoods, req.getParameterMap());
		...
		if(StringUtils.isNotEmpty(mdGoods.getFactoryType())){
			queryWrapper.eq("factory_type",mdGoods.getFactoryType());
		}
	...
	@AutoLog(value = "商品信息-分页列表查询")
	@ApiOperation(value="商品信息-分页列表查询", notes="商品信息-分页列表查询")
	@GetMapping(value = "/listMatrial")
	public Result<?> listMatrial(MdGoods mdGoods,
								   @RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
								   @RequestParam(name="pageSize", defaultValue="10") Integer pageSize,
								   HttpServletRequest req) {
		QueryWrapper<MdGoods> queryWrapper = new QueryWrapper<>();
		...
		queryWrapper.and(
				wrapper->{
					wrapper.eq("factory_type","MATERIAL").or().eq("factory_type","PACKAGING");
				}
		);
		queryWrapper.orderByDesc("create_time");
		Page<MdGoods> page = new Page<MdGoods>(pageNo, pageSize);
	...
	@AutoLog(value = "商品信息-分页列表查询")
	@ApiOperation(value="商品信息-分页列表查询", notes="商品信息-分页列表查询")
	@GetMapping(value = "/listpltn")
	public Result<?> listpltn(MdGoods mdGoods,
								   @RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
								   @RequestParam(name="pageSize", defaultValue="10") Integer pageSize,
								   HttpServletRequest req) {
		QueryWrapper<MdGoods> queryWrapper = QueryGenerator.initQueryWrapper(mdGoods, req.getParameterMap());
		...
		for (MdGoods record : records) {
			String shpYanSeCode = record.getShpYanSeCode();
			//商品颜色
			List<WmsMdgoodColor> list = wmsMdgoodColorService.lambdaQuery().eq(WmsMdgoodColor::getColorCode, shpYanSeCode).list();
			if (CollectionUtil.isNotEmpty(list)){
				WmsMdgoodColor wmsMdgoodColor = list.get(0);
				record.setShpYanSe(wmsMdgoodColor.getColorName());
				record.setShpYanEnse(wmsMdgoodColor.getColorNameEn());
			}

			List<BaGoodsType> baGoodsTypes = baGoodsTypeService.lambdaQuery().eq(BaGoodsType::getGoodsTypeName, record.getChpShuXing()).list();
			if (CollectionUtil.isNotEmpty(baGoodsTypes)){
				BaGoodsType baGoodsType = baGoodsTypes.get(0);
				record.setGoodsTypeEnname(baGoodsType.getGoodsTypeEnname());
			}
		}
		return Result.ok(pageList);
	}
	...
	public Result<?> add(@RequestBody MdGoods mdGoods) {
		...
		if (StringUtils.isEmpty(mdGoods.getJshDanWei())){
	...
	public Result<?> addgoods(@RequestBody MdGoods mdGoods) {
		...
		if (mdGoodsService.lambdaQuery().eq(MdGoods::getShpBianMa,mdGoods.getShpBianMa()).count() > 0) {
			...
			for (MdGoodsItem goodsItem : goodsItems) {
				boolean b = mdGoodsItemService.updateById(goodsItem);
				result.setSuccess(b);
			}
		}else {
			List<MdGoodsItem> goodsItems = mdGoods.getGoodsItems();
			...
			result = mdGoodsService.add(mdGoods);
		}
	...
	@AutoLog(value = "商品信息-分页列表查询")
	@ApiOperation(value="商品信息-分页列表查询", notes="商品信息-分页列表查询")
	@GetMapping(value = "/ItemgoodsDetail")
	public Result<?> ItemgoodsDetail(MdGoodsItem mdGoodsItem,
									HttpServletRequest req) {
		QueryWrapper<MdGoodsItem> queryWrapper = QueryGenerator.initQueryWrapper(mdGoodsItem, req.getParameterMap());
		List<MdGoodsItem> pageList = mdGoodsItemService.list(queryWrapper);
		List<MdGoods> mdGoods2 = new ArrayList<>();
		for (MdGoodsItem goodsItem : pageList) {
			QueryWrapper<MdGoods> queryWrapper2 = new QueryWrapper();
			queryWrapper2.lambda().eq(MdGoods::getShpBianMa,goodsItem.getSttr2());
			MdGoods mdGoods = mdGoodsService.getOne(queryWrapper2);
			mdGoods.setChlShl(goodsItem.getNum2());
			mdGoods2.add(mdGoods);
		}
		return Result.OK(mdGoods2);
	}
...
//        System.err.println("一号没走通");
		...
		// Step.2 获取导出数据
		List<MdGoods> pageList = mdGoodsService.list(queryWrapper);
		//System.err.println(pageList.get(0));
		List<MdGoods> exportList = null;
		...
		for (MdGoods goods : exportList) {
			QueryWrapper<BaGoodsType> queryWrapper2 = new QueryWrapper<>();
			queryWrapper2.lambda().eq(BaGoodsType::getGoodsTypeCode,goods.getChpShuXing());
			BaGoodsType one = baGoodsTypeService.getOne(queryWrapper2, false);
			if (one != null){
				goods.setCfWenCeng(one.getGoodsTypeEnname());
			}
		}
...
//		return super.exportXls(request, mdGoods, MdGoods.class, "商品信息");
	...
	private String getId(MdGoods item) {
		try {
...
//		}
    ...
    */
	...
	public ModelAndView exportXlsgs(HttpServletRequest request, MdGoods mdCus) {
		// Step.1 组装查询条件
		QueryWrapper<MdGoods> queryWrapper = QueryGenerator.initQueryWrapper(mdCus, request.getParameterMap());
		LoginUser loginUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
		System.err.println(loginUser);
		LoginUser sysUser = JwtUtil.getLoginUser();
		List<MdGoods> pageList = mdGoodsService.list(queryWrapper);
		...
		List<MdGoodsgs> list = new ArrayList<MdGoodsgs>();
		for (MdGoods main : exportList) {
			MdGoodsgs vo = new MdGoodsgs();
			BeanUtils.copyProperties(main, vo);
			list.add(vo);
		}
		ModelAndView mv = new ModelAndView(new JeecgEntityExcelView());
		mv.addObject(NormalExcelConstants.FILE_NAME, "商品");
		mv.addObject(NormalExcelConstants.CLASS, MdGoodsgs.class);
		mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("商品", "导出人:" + sysUser.getRealname(), "商品"));
	...
	public Result<?> importExcelgs(HttpServletRequest request, HttpServletResponse response) {
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				List<MdGoodsgs> list = ExcelImportUtil.importExcel(file.getInputStream(), MdGoodsgs.class, params);
				long start = System.currentTimeMillis();
				List<MdGoods> listsave = new ArrayList<>();
				if (list.size() > 0) {
					for(MdGoodsgs mdGoodsgs : list){
						if(org.apache.commons.lang3.StringUtils.isEmpty(mdGoodsgs.getShpBianMa())){
							continue;
						}
						List<MdGoods> list1 = mdGoodsService.lambdaQuery().in(MdGoods::getShpBianMa, mdGoodsgs.getShpBianMa()).list();
						for (MdGoods kw : list1) {
							mdGoodsService.removeById(kw.getId());
						}
						MdGoods mdGoods = new MdGoods();
						BeanUtil.copyProperties(mdGoodsgs,mdGoods);
						listsave.add(mdGoods);
					}
				}
				mdGoodsService.saveBatch(listsave);
	...
	public Result<?> importExcel1(HttpServletRequest request, HttpServletResponse response) {
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			params.setNeedSave(true);
			try {
				List<MdGoods> list = ExcelImportUtil.importExcel(file.getInputStream(), MdGoods.class, params);
				long start = System.currentTimeMillis();
				List<MdGoods> listsave = new ArrayList<>();
				if (list.size() > 0) {
					for(MdGoods mdGoods : list){
						if(StringUtils.isEmpty(mdGoods.getShpBianMa())){
							continue;
						}
						List<MdGoods> list1 = mdGoodsService.lambdaQuery().in(MdGoods::getShpBianMa, mdGoods.getShpBianMa()).list();
						for (MdGoods kw : list1) {
							mdGoodsService.removeById(kw.getId());
						}
						listsave.add(mdGoods);
	 ...
	 * @param mdgoods
	 */
	@RequestMapping(value = "/downloaditemXls")
	public ModelAndView downloaditemXls(HttpServletRequest request, MdGoods mdgoods) {
		//return super.exportXls(request, mdGoods, MdGoods.class, "商品信息");
		// Step.1 组装查询条件
		String selections = request.getParameter("selections");
		QueryWrapper<MdGoods> queryWrapper = QueryGenerator.initQueryWrapper(mdgoods, request.getParameterMap());
		...
		if(StringUtils.isNotEmpty(selections)){
			queryWrapper.lambda().in(MdGoods::getId,selections.split(","));
		}
		List<MdGoods> pageList = mdGoodsService.list(queryWrapper);

		List<MdGoodsItem> mdgoodsitem = new ArrayList<>();
		for (MdGoods mdGoods : pageList) {
			MdGoodsItem mdGoodsItem = new MdGoodsItem();
			mdGoodsItem.setSttr1(mdGoods.getShpBianMa());
			QueryWrapper<MdGoodsItem> queryWrapper2 = QueryGenerator.initQueryWrapper(mdGoodsItem, request.getParameterMap());
			List<MdGoodsItem> pageList2 = mdGoodsItemService.list(queryWrapper2);
			mdgoodsitem.addAll(pageList2);
		}
...
//		}
		...
		// Step.3 AutoPoi 导出Excel
		ModelAndView mv = new ModelAndView(new JeecgEntityExcelView());
		mv.addObject(NormalExcelConstants.FILE_NAME, "商品BOM信息"); //此处设置的filename无效 ,前端会重更新设置一下
		mv.addObject(NormalExcelConstants.CLASS, MdGoodsItem.class);
		mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("商品BOM", "导出人:" + sysUser.getRealname(), "商品信息"));
		mv.addObject(NormalExcelConstants.DATA_LIST, mdgoodsitem);
		return mv;
		//return super.exportXls(request, mdGoods, MdGoods.class, "商品信息");
	}
