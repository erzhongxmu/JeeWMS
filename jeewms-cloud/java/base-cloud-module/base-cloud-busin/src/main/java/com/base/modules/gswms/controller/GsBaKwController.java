...
@Api(tags="储位定义")
@RestController
@RequestMapping("/gswms/baKw")
@Slf4j
public class GsBaKwController extends JeecgController<BaKw, IBaKwService> {
	@Autowired
	...
	public Result<?> queryPageList(BaKw baKw,
		...
		queryWrapper.orderByAsc("ku_wei_bian_ma");
		...
		// TODO: 2021/12/30  选择储位，根据类型判断产品类型
			if (baKwList != null && baKwList.size() > 0) {
				for (BaKw baKw : baKwList) {
					resultMap.put(baKw.getKwBarCode(),baKw.getKwBarCode());
				}
			}
//		}
