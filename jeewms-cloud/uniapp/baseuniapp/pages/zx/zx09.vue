<template>
	<view class="content">
		...
		<view class="Search" style="flex-direction: column;">
			<input type="text" v-model="query01" :focus="isFocus1"  @confirm="getDetails(1)" :placeholder="$t('pages.PleaseScanTheDialOrder')" />
			<input type="text" v-model="query02" :focus="isFocus2"  @confirm="getDetails(1)" :placeholder="$t('pages.PleaseScanTheStorageInformation')" style="margin-top: 16rpx;" />
		</view>
		<scroll-view scroll-y="true" style="flex: 1;overflow: auto;" @scrolltolower='scroll'>
			...
			<view class="item">
				<view>{{$t('pages.batchNumber')+'/'+ $t('pages.dateInProduced')}}：</view>
			...
			<view class="item">
				<view>{{$t('pages.inventory')}}：</view>
				<view style="color: #2D2F2E">{{item.query09}}</view>
			</view>
			<view class="inputText">
				<view class="" style="display: flex; width: 100%; align-items: center;justify-content: space-between;">
					<text>{{$t('pages.AllocateWarehouse')}}：</text>
					<input v-model="item.query10"  style=" color: #3297E2;text-align: right;padding-right: 20rpx;width: 300rpx;margin-top: 0;"  value=""  placeholder-style="color:#E0E2E4"/>
				</view>
			</view>
			<view class="inputText">
				<view class="" style="display: flex; width: 100%; align-items: center;justify-content: space-between;">
					<text>{{$t('pages.allocatingStoring')}}：</text>
					<input  v-model="item.query11"  style=" color: #3297E2;text-align: right;padding-right: 20rpx;width: 300rpx;margin-top: 0;"  value=""  placeholder-style="color:#E0E2E4"/>
				</view>
			</view>
			<view class="inputText"  style="border-bottom: 1px dashed #DBDBDB;">
				<view class="" style="display: flex; width: 100%; align-items: center;justify-content: space-between;">
					<text>{{$t('pages.allocatingTheNumber')}}：</text>
					<input v-model="item.query12"  style=" color: #3297E2;text-align: right;padding-right: 20rpx;width: 300rpx;margin-top: 0;" type="number" value=""  placeholder-style="color:#E0E2E4"/>
				</view>
			</view>
			<view class="btn" @click="preserve(item)">{{$t('pages.allot')}}</view>
		</view>
		</scroll-view>
	</view>
</template>
