<template>
	<view class="content">
		...
		<view class="Search" style="flex-direction: column;">
			<input type="text" v-model="query02" :focus="isFocus1"  @confirm="getDetails(1)" :placeholder="$t('pages.PleaseScanTheStorageInformation')" />
			<input type="text" v-model="query01" :focus="isFocus2" @confirm="getDetails(1)" :placeholder="$t('pages.PleaseScanTheProduct')" style="margin-top: 16rpx;" />
		</view>	
		<scroll-view scroll-y="true" style="flex: 1;overflow: auto;" @scrolltolower='scroll'>
			<view class="body_t" v-for="(item,index) in data" :key=	'index' v-if="Number(item.query09) > 0">
				<view class="item">
					<view>{{$t('pages.TheShipperCode')}}：</view>
				...
				<!-- <view class="item">
					<view>规格{{$t('pages.productStandard')}}：</view>
					<view style="color: #2D2F2E;">{{item.query05}}</view>
				</view> -->
				<!-- <view class="item">
					<view>仓库{{$t('pages.warehouse')}}：</view>
				...
				<view class="inputText"  style="border-bottom: 1px dashed #DBDBDB;">
					<view class="" style="display: flex; width: 100%; align-items: center;justify-content: space-between;">
						<text>{{$t('pages.NumberOfDump')}}：</text>
						<input v-model="item.query11" style=" color: #3297E2;text-align: right;padding-right: 20rpx;width: 300rpx;margin-top: 0;" type="number" value=""  placeholder-style="color:#E0E2E4"/>
					</view>
				</view>
				<view class="btn"  @click="preserve(item)">{{$t('pages.memoryTransfer')}}</view>
			</view>
		</scroll-view>
	</view>
</template>
