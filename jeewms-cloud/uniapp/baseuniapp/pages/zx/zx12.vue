	...
	<view class="content">
		...
		<scroll-view scroll-y="true" style="flex: 1;overflow: auto;" @scrolltolower='scroll'>
			<view class="body_t" v-for="(item,index) in data" :key=	'index'>
				<view class="item">
					<view>{{$t('pages.InventoryNumber')}}：</view>
...
<!-- 				<view class="item">
					<view>盘点仓库{{$t('pages.InventoryNumber')}}：</view>
				...
				<view class="inputText">
					<view class="" style="display: flex; width: 100%; align-items: center;justify-content: space-between;">
						<text>{{$t('pages.physicalInventory')}}：</text>
						<input v-model="item.query09" style=" color: #3297E2;text-align: right;padding-right: 20rpx;width: 300rpx;margin-top: 0;" type="number" value=""  placeholder-style="color:#E0E2E4"/>
					</view>
				</view>
				<view class="btn"  @click="preserve(item)">{{$t('pages.check')}}</view>
			</view>
		</scroll-view>
	</view>
</template>
