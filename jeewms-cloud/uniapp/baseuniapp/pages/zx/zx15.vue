		...
		<scroll-view scroll-y="true" style="flex: 1;overflow: auto;" @scrolltolower='scroll'>
			<view class="body_t" v-for="(item,index) in data" :key=	'index'>
				<view class="title" style="border-bottom: 1px dashed #DBDBDB;">
					...
					{{$t('pages.DeliveryOrderNo')}}：{{item.query01}}
				</view>
				<view class="item">
				...
				<view class="item">
					<view>{{$t('pages.dateInProduced')}}：</view>
				...
				<view class="item">
					<view>{{$t('pages.storageLocation')}}：</view>
					<view style="color: #2D2F2E;">{{item.query10}}</view>
				</view>
				<view class="item">
				...
				<!-- <view class="inputText">
					<view class="" style="display: flex; width: 100%; align-items: center;justify-content: space-between;">
						...
						<view class="select-input" >
							...
							<picker class="search-picker" @change="(e)=>{bindPickerChange(e,index,'query08')}"  :range="item.list01" range-key="query01">
				...
				<!-- <view class="inputText">
					<view class="" style="display: flex; width: 100%; align-items: center;justify-content: space-between;">
						<text>储位库存：</text>
						<view class="select-input" >
							<input v-model="item.query09" style=" color: #3297E2;text-align: right;padding-right: 20rpx;margin-top: 0;"   placeholder-style="color:#E0E2E4"/>
							<picker class="search-picker" @change="(e)=>{bindPickerChange(e,index,'query09')}"  :range="item.list02" range-key="query01">
								<image style="width: 13px; height: 7px;" src="@/static/icon/jiantou.png" mode=""></image>
							</picker>
						</view>
					</view>
				...
				<!-- <view class="inputText" style="border-bottom: 1px dashed #DBDBDB;">
					<view class="" style="display: flex; width: 100%; align-items: center;justify-content: space-between;border">
						<text>托盘号：</text>
						<input v-model="item.query11" style=" color: #3297E2;text-align: right;padding-right: 20rpx;width: 300rpx;margin-top: 0;"   placeholder-style="color:#E0E2E4"/>
					</view>
				</view> -->
				<view class="btn" @click="preserve(item)">{{$t('pages.soldOut')}}</view>
			</view>
		</scroll-view>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			//入库标签打印
			getPrint(e){
				this.$http.get('/jeewms/wmOmQmI/labelPrinting',{params:{id:e}}).then(res => {})
			},
			getDetails(e){
				// if(!this.query01) return uni.showToast({title:'请扫描单号'})
				if(e == 1){
					this.data = []
					this.isFocus1 =  false
					this.isFocus2 =  true
			...
			bindPickerChange(e,i,key){
				let index = e.detail.value;
				this.$nextTick(()=>{
					this.$set(this.data[i], key, this.array[index].query01)
				})
			}
		}
	}
