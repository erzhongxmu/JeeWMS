<template>
	<view class="content">
		...
		<view class="Search" style="flex-direction: column;height: 	177rpx;">
			<input type="text" v-model="query01" :focus="isFocus1"  @confirm="getDetails(1)" :placeholder="$t('pages.PleaseScanTheWaveOrderNumber')"  />
		...
		<scroll-view scroll-y="true" style="flex: 1;overflow: auto;" @scrolltolower='scroll'>
			<view class="body_t" v-for="(item,index) in data" :key=	'index'>
				<view class="title" style="border-bottom: 1px dashed #DBDBDB;">
					...
					{{$t('pages.WaveNumber')}}：{{item.query11}}
				</view>
				<view class="item">
					<view>{{$t('pages.SalesOrder')}}：</view>
				...
				<view class="btn" @click="preserve(item)">{{$t('pages.WaveOffTheShelves')}}</view>
			</view>
		</scroll-view>
	</view>
</template>
