<template>
	<view class="content">
		...
		<view class="Search" style="flex-direction: column;"> 
			<!--  placeholder="请输入领料单号" -->
			<input type="text" v-model="query19" :focus="isFocus1"  :placeholder="$t('pages.PleaseScanTheOrderNumber')" />
			<input type="text" v-model="query20" :focus="isFocus2"  :placeholder="$t('pages.PleaseEnterTheReceivingDepartment')" style="margin-top: 16rpx;" />
		...
		<scroll-view scroll-y="true" style="flex: 1;overflow: auto;" @scrolltolower='scroll'>
			...
			<view class="inputText"  style="border-bottom: 1px dashed #DBDBDB;">
				<view class="" style="display: flex; width: 100%; align-items: center;justify-content: space-between;">
					...
					<input v-model="item.query10"  style=" color: #3297E2;text-align: right;padding-right: 20rpx;width: 300rpx;margin-top: 0;" type="number" value=""  placeholder-style="color:#E0E2E4"/>
				</view>
			</view>
			<view class="btn" @click="preserve(item)">{{$t('pages.Picking')}}</view>
		</view>
		</scroll-view>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			preserve(item){
				if(this.query19==''||this.query19==null){
					uni.showToast({ title:'领料单号不能为空',icon:'none' })
					return
				}
				if(this.query20==''||this.query20==null){
					uni.showToast({ title:'领料部门不能为空',icon:'none' })
					return
				}
				let data = {
					...item,
					...
					listtype:this.title,
					query19:this.query19,
					query20:this.query20
				}
