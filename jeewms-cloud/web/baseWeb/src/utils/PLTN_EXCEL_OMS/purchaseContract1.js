...
export function exportFile(header, columns, dataList, expertName) {
  ...
  // 设置顶部样式
  let styleArr = ['A', 'B', 'E', 'F']
  let styleRow = ['2','3', '4', '5', '6']
  styleArr.map((item, index) => {
    styleRow.map((v, i) => {
      worksheet.getCell(`${item}${v}`).font = {
        name: 'Arial',
        size: 11,
        underline: false,
        bold: false
...
2.开票人跟复核人不可以是同一个人`
  ...
  data.map((item, index) => {
    ...
    arr[6] = item.query16
    arr[7] = item.query21
    console.log(item.query18,item.query16,item.query18 - item.query16);
