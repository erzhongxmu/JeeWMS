...
export function exportFile2(header, columns, dataList, expertName) {
  ...
  worksheet.getCell('A1').value = '采购合同'
  ...
  worksheet.getCell('I6').value = topData.query28
  ...
  arr1.map((item, index) => {
    arr2.map((v, index) => {
      ...
      if (item == 'C' || item == 'D' || item == 'E' || item == 'F') {
  ...
  worksheet.getCell('H' + row).value = '样品可退金额'
  worksheet.getCell('I' + row).value = topData.query27
  worksheet.mergeCells(`A${row}:F${row+2}`)
  // 样品可退金额加边框、设置高
  ...
  // 其他费用
  worksheet.mergeCells(`I${row}:J${row}`)
  worksheet.getCell('H' + row).value = '其他费用'
  worksheet.getCell('I' + row).value = topData.query29
  ...
  worksheet.getRow(row).height = 30
  worksheet.getCell(`G${row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true }
  worksheet.getCell(`H${row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true }
  // 生成商品信息表
  worksheet.addTable({
    name: 'MyTable1',
...
数量`, filterButton: false
      ...
      { name: `UNIT PRICE (${topData.query25}) 单价 `, filterButton: false },
//       {
