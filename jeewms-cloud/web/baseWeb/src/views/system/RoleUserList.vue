      ...
      <a-card :bordered="false">
        ...
        <div class="table-page-search-wrapper">
          ...
          <a-form layout="inline" @keyup.enter.native="searchQuery">
            <a-row :gutter="24">
              <a-col :md="12" :sm="8">
                <a-form-item :label="$t('角色名称')" :labelCol="{span: 5}" :wrapperCol="{span: 18, offset: 1}">
                  <a-input :placeholder="$t('请输入')" v-model="queryParam.roleName"></a-input>
                </a-form-item>
              </a-col>
              <!--
              <a-col :md="11" :sm="12">
            ...
            <a-col :md="12" :sm="24">
               <a-button type="primary" @click="searchQuery" icon="search" style="margin-left: 21px">{{$t('查询')}}</a-button>
        ...
        <div class="table-operator" style="margin: 5px 0 10px 2px">
          ...
          <a-button type="primary" icon="download" @click="handleExportXls($('角色管理'))">{{$t('导出')}}</a-button>
        </div>

        <div class="ant-alert ant-alert-info" style="margin-bottom: 16px;">
          <i class="anticon anticon-info-circle ant-alert-icon">
          </i> {{$t('已选择')}} <a><b>{{ selectedRowKeys1.length }}</b></a>{{$t('项目')}}
          <a style="margin-left: 24px" @click="onClearSelected1">{{$t('清空')}}</a>
        </div>
        ...
        <div style="margin-top: 15px">
          ...
          <span slot="action" slot-scope="text, record">
            <a @click="handleOpen(record)">{{$t('用户')}}</a>
            ...
            <a-dropdown>
              ...
              <a-menu slot="overlay">
                <a-menu-item>
                  <a @click="handlePerssion(record.id)">{{$t('授权')}}</a>
                </a-menu-item>
                <a-menu-item>
                  <a @click="handleEdit(record)">{{$t('编辑')}}</a>
                </a-menu-item>
                <a-menu-item>
                  <a-popconfirm :title="$t('你确定吗?')" @confirm="() => handleDelete1(record.id)">
                    <a>{{$t('删除')}}</a>
                  </a-popconfirm>
                </a-menu-item>
              </a-menu>
    ...
    <a-col :md="rightColMd" :sm="24" v-if="this.rightcolval == 1">
      <a-card :bordered="false">
        ...
        <div class="table-page-search-wrapper">
          <a-form layout="inline">
            <a-row :gutter="24">
              ...
              <span style="float: left;overflow: hidden;" class="table-page-search-submitButtons">
            <a-col :md="9" :sm="24">
             <a-button type="primary" @click="searchQuery2" icon="search" style="margin-left: 21px">{{$t('查询')}}</a-button>
              <a-button type="primary" @click="searchReset2" icon="reload" style="margin-left: 8px">{{$t('重置')}}</a-button>
        ...
        <div class="table-operator" :md="24" :sm="24">
          <a-button @click="handleAdd2" type="primary" icon="plus" style="margin-top: 16px">{{$t('新增用户')}}</a-button>
          <!--<a-button @click="handleEdit2" type="primary" icon="edit" style="margin-top: 16px">用户编辑</a-button>-->
          <a-button @click="handleAddUserRole" type="primary" icon="plus" style="margin-top: 16px">{{$t('已有用户')}}</a-button>
        ...
        <div>
          <div class="ant-alert ant-alert-info" style="margin-bottom: 16px;">
            ...
            selectedRowKeys2.length }}</a>{{$t('项目')}}
            <a style="margin-left: 24px" @click="onClearSelected2">{{$t('清空')}}</a>
          </div>
          <a-table
            style="height:500px"
            ref="table2"
           ...
           <span slot="action" slot-scope="text, record">
           <a @click="handleEdit2(record)">{{$t('编辑')}}</a>
          <a-divider type="vertical"/>
          <a-dropdown>
            <a class="ant-dropdown-link">
               {{$t('更多')}} <a-icon type="down"/>
            ...
            <a-menu slot="overlay">
              <a-menu-item>
                <a-popconfirm :title="$t('你确定吗?')" @confirm="() => handleDelete2(record.id)">
                  <a>{{$t('删除')}}</a>
                </a-popconfirm>
              </a-menu-item>
            </a-menu>
