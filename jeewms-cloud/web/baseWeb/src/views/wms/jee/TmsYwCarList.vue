...
<!--      </a-dropdown>-->
      <a-button v-if="selectedRowKeys.length > 0" @click="getCarList" type="primary" icon="car">批量派车</a-button>
    </div>
    ...
    <j-modal
      ...
      <a-spin :spinning="confirmLoading">
        <a-form :form="form">
          <a-form-item label="派车司机" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-select
              v-decorator="['car',validatorRules.car]"
              labelInValue
              placeholder="请选择车辆"
              @select="handleSelectChange"
            >
              <a-select-option v-for="(item,key) in carList" :key="key" :value="item.username">
                {{item.chepaihao}}
              </a-select-option>
  ...
  export default {
    ...
    methods: {
          ...
          if(!err) {
            ...
            postAction(this.url.batchSendCar,param).then((res) => {
              if (res.success) {
                ...
                this.loadData();
                this.$nextTick(() => {
                  this.form.setFieldsValue({ //《=关键 清空tag选项
                    'car': []
                  })
                })
              } else {
      ...
      handleCancel() {
        ...
        this.carInfo = {};
        this.$nextTick(() => {
          this.form.setFieldsValue({ //《=关键 清空下拉选项
            'car': []
          })
        })
      }
