<template>
  <a-card class="j-inner-table-wrapper" :bordered="false">
        ...
        <a-row :gutter="24">
          ...
          <a-col :xl="4" :lg="7" :md="8" :sm="24">
            <a-form-item :label="$t('备注')">
              <a-input v-model="queryParam.omBeizhu" :placeholder="$t('请输入')" />
            </a-form-item>
          </a-col>
    ...
    <div class="table-operator">
      <a-button type="primary" icon="plus" @click="handAdd('19')" v-has="'WmOmMessageHList03:add'">{{
        $t('添加')
      }}</a-button>
      <a-button
        type="primary"
        icon="download"
        @click="handleExportXls($t('其他出库'))"
        v-has="'WmOmMessageHList03:export'"
        >{{ $t('导出') }}</a-button
      >
      <a-button
        type="primary"
        icon="download"
        @click="handleExportModal($t('其他出库'))"
        v-has="'WmOmMessageHList03:exportModal'"
        >{{ $t('模板下载') }}</a-button
      >
      ...
      <!--      </a-upload>-->
      <a-upload
        v-has="'WmOmMessageHList03:import'"
        name="file"
      ...
      <!-- <a-button @click="batchDel"><a-icon type="delete" v-has="'WmOmMessageHList03:batchDelete'"/>{{$t('批量删除')}}</a-button>-->
    </div>
    <!-- 操作按钮区域 end -->
    <j-modal
      :visible="isShow"
    ...
    <div>
      ...
      >
        ...
        <template slot="action" slot-scope="text, record">
          <span v-has="'WmOmMessageHList03:finish'" v-if="record.omSta != '已完成'">
            <a-popconfirm title="确定完成吗?" @confirm="() => accomplish(record)">
              <a v-if="record.omSta != '已完成'">{{ $t('完成') }}</a>
            </a-popconfirm>
            <a-divider type="vertical" />
          </span>
          <a @click="handleCheck(record)" v-has="'WmOmMessageHList03:check'">{{ $t('查看') }}</a>
          <a-divider type="vertical" v-has="'WmOmMessageHList03:check'" />
          <a @click="handleEdit(record)" v-has="'WmOmMessageHList03:update'">{{ $t('编辑') }}</a>
          <!-- WMS -->
          <!-- <span>
          ...
          <span>
            <a-divider type="vertical" v-has="'WmOmMessageHList03:update'" v-if="record.omSta !='已完成'" />
            <a @click="GS_Print(record,'JH')" v-has="'WmOmMessageHList03:update'">{{$t('拣货单')}}</a> -->
          <!-- <a-divider type="vertical" v-has="'WmOmMessageHList03:update'" v-if="record.omSta !='已完成'" />
            <a @click="GS_Print(record,'CK')" v-has="'WmOmMessageHList03:update'">出库单</a> -->
          <!-- </span> -->
          <!--<a-divider type="vertical" v-has="'WmOmMessageHList03:update'" />
          <a @click="print(record)" v-has="'WmOmMessageHList03:update'">{{$t('打印')}}</a>
          <a-divider type="vertical" v-has="'WmOmMessageHList03:printOut'" />
          <a @click="openPrint(record,'jh')" v-has="'WmOmMessageHList03:printJh'">拣货单</a>
          <a-divider type="vertical" v-has="'WmOmMessageHList03:printJh'" />
          <a @click="openPrint(record,'jh')" v-has="'WmOmMessageHList03:printZs'">追溯单</a>
          <a-divider type="vertical" v-has="'WmOmMessageHList03:printZs'" />
          <a @click="exportBill('ck',record.id)" v-has="'WmOmMessageHList03:exportOut'">导出出库单</a>
          <a-divider type="vertical" v-has="'WmOmMessageHList03:exportOut'" />
          <a @click="openPrint(record,'ck')" v-has="'WmOmMessageHList03:printOut'">打印出库单</a> -->
          <!-- <a-divider type="vertical" v-has="'WmOmMessageHList03:update'" />
          ...
          <a-dropdown>
            ...
            <a-menu slot="overlay">
              <!-- <a-menu-item>
                <a @click="inspectionSheet(record.id)" v-has="'WmOmMessageHList03:exportAccept'">{{$t('导出领料单')}}</a>
              </a-menu-item> -->
              <a-menu-item>
                <a @click="GS_Print(record, 'JH')" v-has="'WmOmMessageHList03:printJh'">{{ $t('拣货单') }}</a>
              </a-menu-item>
              <a-menu-item>
                <a-popconfirm :title="$t('你确定吗?')" @confirm="() => handleDelete(record.id)">
                  <a v-has="'WmOmMessageHList03:delete'">{{ $t('删除') }}</a>
                </a-popconfirm>
              </a-menu-item>
            </a-menu>
          </a-dropdown>
        ...
        <tbody>
          ...
          <tr height="40" style="mso-height-source: userset; height: 30pt">
            <td colspan="5" height="40" class="xl68" width="242" style="height: 30pt; width: 182pt">
              货主名称：{{ printData.cusname ? printData.cusname : '' }}
            </td>
...
export default {
  ...
  created() {
    let str = "! 0 250 530 505 1\r\nBARCODE QR 10 90 M 2 U 9\r\nMA, $qrcodeId#\r\nENDQR\r\nSETMAG 1 2\r\nTEXT 8 0 260 55 品名: $sampleNumber# \r\nTEXT 8 0 260 10颜色:$sampleColor#\r\nTEXT 8 0 260 155 批号:$batchNumber#\r\n TEXT 8 0 260 205 厂家色号: $supplierColorNumber#\r\nTEXT 8 0 260 255 备注:$remark#\r\nTEXT 8 0 260 305 数量:$orderWeight#\r\nSETMAG 1 1\r\nTEXT 8 0 65 320 $qrcodeId#\r\nPRINT\r\n"

  },
  methods: {
    searchReset() {
      this.queryParam = {orderTypeCode: "19"}
      this.loadData()
    },
    accomplish(e) {
    ...
    inspectionSheet(id) {
      getAction('/jeewms/wmOmNoticeH/queryplqnById', { id: id }).then((res) => {
