...
<!--      </a-col>-->
    ...
    <div class="table-operator">
      <a-button type="primary" icon="plus" @click="handleAdd" v-has="'omClient:add'">{{$t('添加')}}</a-button>
      <a-button type="primary" icon="download" @click="handleExportXls('出库管理')">{{$t('导出')}}</a-button>
      <a-upload name="file" :showUploadList="false" :multiple="false" v-has="'omClient:import'" :headers="tokenHeader" :action="importExcelUrl" @change="handleImportExcel">
        <a-button type="primary" icon="import">{{$t('导入')}}</a-button>
      </a-upload>
      <a-button @click="batchDel" v-has="'omClient:batchDelete'"><a-icon type="delete"/>{{$t('批量删除')}}</a-button>
    </div>
    <!-- 操作按钮区域 end -->
...
<!--        </template>-->
        ...
        <template slot="action" slot-scope="text, record">
          <a @click="handleCheck(record)" v-has="'omClient:check'">{{$t('查看')}}</a>
          <a-divider type="vertical" v-has="'omClient:check'"/>
          <a @click="handleEdit(record)" v-has="'omClient:update'">{{$t('编辑')}}</a>
...
<!--          </a-popconfirm>-->
    ...
    <!-- 表单区域 -->
    <wm-om-client-h-modal ref="modalForm" @ok="modalFormOk"/>
  </a-card>
</template>
...
<script>
  ...
  import WmOmClientHModal from './modules/WmOmClientHModal'
