<template>
  <a-card class="j-inner-table-wrapper" :bordered="false">
    ...
    <div class="table-operator">
      <a-button type="primary" icon="plus" @click="handAdd('14')" v-has="'omMessage:add'">{{$t('添加')}}</a-button>
      <a-button
        type="primary"
        icon="download"
        @click="handleExportXls('出库')"
        v-has="'omMessage:export'"
      >{{$t('导出')}}</a-button>
      <a-button
        type="primary"
        icon="download"
        @click="handleExportModal('出库')"
        v-has="'omMessage:exportModal'"
      >{{$t('模板下载')}}</a-button>
      <!--      <a-upload name="file" :showUploadList="false" :multiple="false" :headers="tokenHeader" :action="importExcelUrl" @change="handleImportExcel">-->
      <!--        <a-button type="primary" icon="import">{{$t('导入')}}</a-button>-->
      <!--      </a-upload>-->
      <a-upload
        v-has="'omClient:import'"
        name="file"
    ...
    <div>
      ...
      >
        ...
        <template slot="action" slot-scope="text, record">
          <a @click="handleCheck(record)" v-has="'omMessage:check'">{{$t('查看')}}</a>
          <a-divider type="vertical" v-has="'omMessage:check'" />
          <a @click="handleEdit(record)" v-has="'omMessage:update'">{{$t('编辑')}}</a>
