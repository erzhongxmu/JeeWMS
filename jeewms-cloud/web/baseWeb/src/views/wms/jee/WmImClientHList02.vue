  ...
  <a-card :bordered="false">
        ...
        <a-row :gutter="24">
          ...
          <a-col :xl="4" :lg="7" :md="8" :sm="24">
            <a-form-item :label="$t('批次')">
              <a-input :placeholder="$t('请输入')" v-model="queryParam.whereCon"></a-input>
            </a-form-item>
          </a-col>
          <a-col :xl="4" :lg="7" :md="8" :sm="24">
    ...
    <div class="table-operator">
      <a-button
        ...
        type="primary"
        icon="plus"
        v-has="'WmImClientHList02:add'"
      >{{$t('添加')}}</a-button>
      <a-button type="primary" icon="download" @click="handleExportXls($t('收货订单'))" v-has="'WmImClientHList02:export'">{{$t('导出')}}</a-button>
      <a-upload
        name="file"
      ...
      >
        <a-button type="primary" icon="import" v-has="'WmImClientHList02:import'">{{$t('导入')}}</a-button>
      </a-upload>
    ...
    <div>
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          <span v-has="'WmImClientHList02:finish'">
            <a-popconfirm title="确定完成吗?" @confirm="() => accomplish(record)">
              <a v-if="record.imSta=='计划中'">{{$t('完成')}}</a>
            </a-popconfirm>
            <a-divider type="vertical" v-has="'imClient:edit'" v-if="record.imSta=='计划中'" />
          </span>
          <span v-has="'WmImClientHList02:check'">
            <a @click="handleCheck(record)" >{{$t('查看')}}</a>
            <a-divider type="vertical" v-if="record.imSta=='计划中'" />
          </span>
          <span v-has="'WmImClientHList02:edit'">
          ...
          <!-- <span>
            ...
            <a-divider type="vertical" v-if="record.imSta=='计划中'" />-->
            <!-- <a @click="UploadIMG(record)" v-if="record.imSta=='计划中'">{{$t('上传生产图片')}}</a>
            <a-divider type="vertical" v-has="'imClient:printOrder'" />
            <a @click="goPrintOrder(record)" v-has="'imClient:printOrder'">{{$t('打印通知单')}}</a>
          </span> -->
          ...
          <a-dropdown>
            ...
            <a-menu slot="overlay">
              <a-menu-item>
                <a @click="UploadIMG(record)" v-if="record.imSta=='计划中'" v-has="'WmImClientHList02:uploadImg'">{{$t('上传生产图片')}}</a>
              </a-menu-item>
              <a-menu-item>
                <a @click="goPrintOrder(record)" v-has="'WmImClientHList02:printOrder'">{{$t('打印通知单')}}</a>
              </a-menu-item>
              <a-menu-item>
                <a-popconfirm :title="$t('你确定吗?')" @confirm="() => handleDelete(record.id)">
                  <a v-has="'WmImClientHList02:delete'">{{$t('删除')}}</a>
                </a-popconfirm>
              </a-menu-item>
            </a-menu>
          </a-dropdown>
...
import { INSPECTION_SHEET, STOCK_REQUISITION } from '@/utils/PLTN_EXCEL/index' // WMS导出检验单，包装领料单
...
export default {
  ...
  methods: {
    // WMS-检验单导出
    searchReset() {
      this.queryParam = {orderTypeCode: "07"}
      this.loadData()
    },
    inspectionSheet(id) {
