    ...
    <a-table :scroll="{x:2200,y:500}" bordered
      ...
      <a-table-column title="操作" dataIndex=""  align="center" :width="250" fixed='right'>
        <template slot-scope="t,r,i">
          ...
          <br>
          <a href="javascript:void(0);"  @click="edit(r)">{{$t('编辑')}}</a> <a-divider type="vertical" />
          <a href="javascript:void(0);" style="color: red;" @click="remove(r)">{{$t('删除')}}</a>
      ...
      sprySubmit() {
        ...
        this.postFormAction(_this.url.editNodeUser,this.spry).then(res => {
          if (res.success) {
            ...
            _this.$message.error(this.$t('操作失败'));
          }
