<template>
  <a-card :bordered="false">
      ...
      <a-form layout="inline" @keyup.enter.native="searchQuery">
        <a-row :gutter="24">
          ...
          <span style="float: left;overflow: hidden;" class="table-page-search-submitButtons">
            <a-col :md="6" :sm="12" >
                <a-button type="primary"  style="left: 10px" @click="searchQuery" icon="search">{{$t('查询')}}</a-button>
                <a-button type="primary"  @click="searchReset" icon="reload" style="margin-left: 8px;left: 10px">{{$t('重置')}}</a-button>
            </a-col>
          </span>
      ...
      <a-table-column title="操作" dataIndex=""  align="center">
        <template slot-scope="t,r,i">
          <template v-if="r.status == 0">
            ...
            <a-divider type="vertical" />
            <a-popconfirm :title="$t('你确定吗?')" @confirm="() => remove(r)">
              <a href="javascript:void(0);"  style="color: red">{{$t('删除')}}</a>
            </a-popconfirm>
          ...
          <template v-else-if="(r.status == 2 && r.result == 3) || r.status == 3">
            ...
            <a-divider type="vertical" />
            <a href="javascript:void(0);" @click="edit(r)" style="color:#000000;">{{$t('编辑')}}</a>
            <a-divider type="vertical" />
    ...
    <!--提交申请表单-->
    <j-modal title="提交申请" v-model="modalVisible" :mask-closable="false" :width="500" :footer="null">
    ...
    <j-modal title="审批历史" v-model="modalLsVisible" :mask-closable="false" :width="'80%'" :footer="null">
      <div v-if="modalLsVisible">
        <historicDetail :procInstId="procInstId"></historicDetail>
      </div>
    </j-modal>
    <j-modal title="确认撤回" v-model="modalCancelVisible" :mask-closable="false" :width="500">
