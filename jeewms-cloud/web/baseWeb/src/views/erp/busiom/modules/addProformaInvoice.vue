<template>
    <j-modal :title="title" :width="width" :visible="visible" switchFullscreen @ok="handleOk" @cancel="close"
        ...
        <j-modal :title="$t('确认开票数据')" :width="800" :visible="visible2" switchFullscreen @ok="handleOk2" @cancel="close2" :maskClosable="false" :confirm-loading="confirmLoading2">
...
export default {
    ...
    methods: {
        ...
        loadData() {
            ...
            getAction("/jeeerp/vOmsPoWq/list", obj).then(res => {
                if (res.success) {
        ...
        handleOk() {
            ...
            getAction("/jeeerp/vOmsPoWq/list2", obj).then(res => {
                if (res.success) {
        ...
        handleOk2(){
            ...
            if(str2) return this.$message.warning('请检查子PO' + str + "开票金额输入格式错误")
            if(str) return this.$message.warning('请检查子PO' + str + "开票金额是否是否大于可开票金额")
            let data2 = {
                postType: "XSFP",
                list01: data
            }
