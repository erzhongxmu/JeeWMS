  ...
  <a-card :bordered="false">
    ...
    <div class="table-page-search-wrapper">
      <a-form layout="inline" @keyup.enter.native="searchQuery">
        <a-row :gutter="24">
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item :label="$t('PO号')">
              <a-input v-model="queryParam.query13" :placeholder="$t('请输入PO号')"></a-input>
            </a-form-item>
          </a-col>
        <a-col :xl="6" :lg="7" :md="8" :sm="24">
        ...
        <!-- <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item :label="$t('领料单号')">
              <a-input :placeholder="$t('请输入单号')" v-model="queryParam.query04"></a-input>
            </a-form-item>
          </a-col>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item :label="$t('加工单号')">
              <a-input :placeholder="$t('请输入单号')" v-model="queryParam.link02"></a-input>
            </a-form-item>
          </a-col> -->
                    <a-col :xl="6" :lg="7" :md="8" :sm="24">
    ...
    <div class="table-operator">
      <!-- <a-button @click="handleAdd" type="primary" icon="plus" v-has="'BusiPrdOrder:add'">新增</a-button> -->
      <a-button type="primary" icon="download" @click="handleExportXls('生产订单')" v-has="'BusiPrdOrder:export'"
        >{{$t('导出')}}</a-button
      >
      <!-- <a-upload
        name="file"
      ...
      >
        <a-button type="primary" icon="import" v-has="'BusiPrdOrder:import'">导入</a-button>
      </a-upload> -->
      <!-- 高级查询区域 -->
      <!-- <j-super-query :fieldList="superFieldList" ref="superQueryModal" @handleSuperQuery="handleSuperQuery"></j-super-query> -->
      <!-- <a-dropdown v-if="selectedRowKeys.length > 0">
        <a-menu slot="overlay">
          <a-menu-item key="1" @click="batchDel" v-has="'BusiPrdOrder:batchDelete'"
    ...
    <div>
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          <a @click="handleEdit(record)" v-has="'BusiPrdOrder:edit'">{{$t('编辑')}}</a>

          <a-divider type="vertical" v-has="'BusiPrdOrder:edit'" />
          <a-dropdown>
            <a class="ant-dropdown-link">{{$t('更多')}} <a-icon type="down" /></a>
            <a-menu slot="overlay">
              <a-menu-item v-has="'BusiPrdOrder:detail'">
                <a @click="handleDetail(record)">{{$t('详情')}}</a>
              </a-menu-item>
              <a-menu-item v-has="'BusiPrdOrder:delete'">
    ...
    <busi-prd-order-modal ref="modalForm" @ok="modalFormOk" />
  </a-card>
</template>

<script>
import { JeecgListMixin } from '@/mixins/JeecgListMixin'
import BusiPrdOrderModal from './modules/BusiPrdOrderModal'
...
import { BusiPrdOrder } from '@/utils/PLTN_EXCEL_OMS/ExportHeader.js'
export default {
  name: 'BusiPrdOrder',
  ...
  methods: {
    handleExportXls(name) {
      ...
      getAction('/jeeerp/ExportTableData/ExportBusiPrdOrder', param).then((res) => {
        ExportTemplate([], BusiPrdOrder, res.result, name)
      })
    },
    searchReset() {
      this.queryParam = { query01: 'SCDD' }
      this.loadData()
    },
    initDictConfig() {},
    getSuperFieldList() {
