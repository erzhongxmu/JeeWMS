<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="24">
          ...
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <span style="float: left;overflow: hidden;" class="table-page-search-submitButtons">
              <a-button type="primary" @click="searchQuery" icon="search" v-has="'BusiPrdOrdList:search'" >{{$t('查询')}}</a-button>
              <a-button type="primary" @click="searchReset" icon="reload" style="margin-left: 8px" v-has="'BusiPrdOrdList:reset'" >{{$t('重置')}}</a-button>
            </span>
          </a-col>
        </a-row>
      </a-form>
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportXls('加工订单')" v-has="'BusiPrdOrdList:export'" >{{$t('导出')}}</a-button>
      ...
      <a-button @click="handleCopy(selectedRowKeys)" type="primary" icon="copy" v-if="selectedRowKeys.length == 1">{{$t('复制')}}</a-button>
    </div>
    ...
    <div>
      ...
      <a-table
        ...
        <span slot="action" slot-scope="text, record"  v-if="record.query02 != '已取消'">
           <a @click="handleEdit2(record,'finish')" v-has="'BusiPrdOrdList:edit'"  v-if="record.query02 != '已完成'">{{$t('完成')}}</a>
            <a-divider type="vertical" v-has="'BusiPrdOrdList:edit'" v-if="record.query02 != '已完成'"/>
          <a @click="handleEdit(record)" v-has="'BusiPrdOrdList:edit'" >{{$t('编辑')}}</a>

          <a-divider type="vertical" v-has="'BusiPrdOrdList:edit'" />
          ...
          <a-dropdown>
            ...
            <a-menu slot="overlay">
            <a-menu-item>
                <a @click="inspectionSheet(record)">{{$t('导出领料单')}}</a>
              </a-menu-item>
              <a-menu-item v-has="'BusiPrdOrdList:detail'" >
                <a @click="handleDetail(record)">{{$t('详情')}}</a>
              </a-menu-item>
              <a-menu-item v-has="'BusiPrdOrdList:quxiao'">
                <a-popconfirm :title="$t('确定取消吗?')" @confirm="() => quxiao(record)">
                  <a>{{$t('取消')}}</a>
                </a-popconfirm>
              </a-menu-item>
              <a-menu-item v-has="'BusiPrdOrdList:delete'" >
    ...
    <busi-prd-ord-modal ref="modalForm" @ok="modalFormOk"/>
...
<script>
  ...
  import BusiPrdOrdModal from './modules/BusiPrdOrdModal'
  ...
  import { BusiPrdOrdList } from '@/utils/PLTN_EXCEL_OMS/ExportHeader.js'
  ...
  export default {
    ...
    data () {
      return { 
        ...
        },
        {
          ...
          align: 'center',
          dataIndex: 'query33',
          NodePermission:'BusiPrdOrdList:query33',
          customRender: (text) => {
        ...
        },
        {
          ...
          sorter: true,
          dataIndex: 'query32',
          NodePermission:'BusiPrdOrdList:query32',
          customRender: (text) => {
            return text?OUT_PUT(text):text
          }
        },
        ...
        url: {
          ...
          exportXlsUrl: "/jeeerp/busiPrdOrd/exportXls2",
    ...
    quxiao(e){
      this.loading = true
      getAction('/jeeerp/busiPrdOrd/updatequery02', { ordId: e.query04,query02:'已取消'}).then(res => {
        this.loading = false
        if (res.success) {
    ...
    printOK(){
      ...
      getAction('/jeewms/baUnit/list', {unitCode:this.printb.query12}).then(res=>{
        ...
        printContent.push({
          ...
          contractlno:this.printb.query13 + 'A',
    ...
    printclose(){
      ...
      inspectionSheet(e){
        getAction('/jeewms/wmOmNoticeH/queryplqnById', {id:e.query04}).then(res=>{
      ...
      //   }
      //   getAction('/jeeerp/ExportTableData/ExportBusiPrdOrdList', param).then((res) => {
      //     ExportTemplate([],BusiPrdOrdList,res.result,name)
      //   })
      // },
      handleEdit2(e,type){
