<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportXls('加工排班')" v-has="'BusiPrdOrdList:export'" >{{$t('导出')}}</a-button>
...
<script>
  ...
  import { BusiPrdOrdList2 } from '@/utils/PLTN_EXCEL_OMS/ExportHeader.js'
  export default {
    name: "BusiPrdOrdList",
    ...
    methods: {
      ...
      getAction(this.url.list, param).then((res) => {
        if (res.success) {
          ExportTemplate([],BusiPrdOrdList2,res.result.records,e)
        }
        if(res.code===510){
      ...
      //   }
      //   getAction('/jeeerp/ExportTableData/ExportBusiPrdOrdList2', param).then((res) => {
      //     ExportTemplate([],BusiPrdOrdList2,res.result,name)
      //   })
      // },
      handleEdit(e){
