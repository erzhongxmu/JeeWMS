<template>
  <a-spin :spinning="confirmLoading">
        ...
        <a-row>
          ...
          <!-- <a-col :span="8">
            <a-form-model-item :label="$t('建单日期')" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="query03">
              <j-date :placeholder="$t('请选择建单日期')" v-model="model.query03" style="width: 100%" disabled/>
            </a-form-model-item>
          </a-col> -->
          <a-col :span="8">
          ...
          <a-col :span="8">
            <a-form-model-item :label="$t('PAYMENT LIST') " :labelCol="labelCol" :wrapperCol="wrapperCol" prop="query19">
              <a-input  v-model="model.query19" :placeholder="$t('请输入PAYMENT LIST') "/>
            </a-form-model-item>
          </a-col>
          <!-- <a-col :span="8">
            <a-form-model-item :label="$t('付款方式')" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="query26">
              <j-dict-select-tag  v-model="model.query26" dictCode="payment_method"  />
            </a-form-model-item>
          </a-col> -->
          <!-- <a-col :span="8">
            <a-form-model-item :label="$t('付款比例')%" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-model="model.query18" :placeholder="$t('请输入付款比例')" @input="query18Input"></a-input>
            </a-form-model-item>
          </a-col>
          <a-col :span="8">
            <a-form-model-item :label="$t('计划付款金额')" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="num02">
              <a-input v-model="model.num02" :placeholder="$t('请输入计划付款金额')" ></a-input>
            </a-form-model-item>
          </a-col>
          <a-col :span="8"  v-if="model.query25 == '是'">
            <a-form-model-item :label="$t('计划付款含税金额')" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="num03">
    ...
    <a-tabs v-model="activeKey" @change="handleChangeTabs">
      <a-tab-pane :tab="$t('付款计划商品信息')" :key="refKeys[0]" :forceRender="true">
        <j-editable-table
          :ref="refKeys[0]"
          :loading="busiPoPlanItem.loading"
          :columns="model.query25 == '是'?busiPoPlanItem.columns:busiPoPlanItem.columns2"
...
export default {
  ...
  data() {
    return {
      ...
      validatorRules: {
          ...
          num02: [{ required: true, message: ' ' }],
          query26: [{ required: true, message: ' ' }],
      },
  ...
  methods: {
    query18Input(){
      this.model.num02 = Number(this.model.num08) * (Number(this.model.query18) / 100)
      this.model.num03 = Number(this.model.num09) * (Number(this.model.query18) / 100)
            ...
            tables[0].setValues([
              {
                rowKey: item.id, // 行的id
                values:{
                  num14:Number(item.num05) * (Number(this.model.query18) / 100),
                  num15:Number(item.num07) * (Number(this.model.query18) / 100),
                }
              }
            ])
          })
    ...
    getPaymentSchedule(orderId) {
      ...
      getAction('/jeeerp/busiPo/list', params).then((res) => {
        ...
        if (this.paymentSchedule.length) {
          ...
          this.model.id = ''
          this.model.query26 = ''
          this.model.num08 = this.model.num11
          this.model.num09 = this.model.num12
    ...
    classifyIntoFormData(allValues) {
      ...
      allValues.tablesValue[0].values.map((item, index) => {
        list.push({
          ...
          ...item,
          query01: 'CGFKJH',
          link01: this.model.query04?'采购订单':null,
        })
      })
      return list
    },
