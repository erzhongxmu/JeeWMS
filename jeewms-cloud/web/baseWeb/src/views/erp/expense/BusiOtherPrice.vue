<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportXls('其他费用')">{{$t('导出')}}</a-button>
      <a-upload
        name="file"
    ...
    <div>
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          <a @click="handleEdit(record)" v-if="record.query23 != '已确认'">{{$t("编辑")}}</a>
          <a-divider type="vertical" />
          <a-popconfirm title="确定删除吗?" @confirm="() => handleDelete(record.id)">
            <a v-has="'BusiOtherPrice:SC'">删除</a>
          </a-popconfirm>
          <a-divider type="vertical" />
          <a-popconfirm :title="$t('确定已付款吗?')" @confirm="() => ConfirmedPayment(record.id)">
            <a v-if="record.query23 != '已确认'">{{$t("已付款")}}</a>
          </a-popconfirm>

<a-divider type="vertical" />

<a @click="handleEdit(record)"  v-has="'BusiOtherPrice:BJ'" >{{$t("编辑")}}</a>

</span>
      </a-table>
    </div>

    <busi-other-price-modal ref="modalForm" @ok="modalFormOk" />
  </a-card>
</template>
...
import BusiOtherPriceModal from './modules/BusiOtherPriceModal'
  ...
  data() {
    return {
      ...
      url: {
        ...
        importExcelUrl: 'jeeerp/busiOrdPrice/importQTFY'
      },
      dictOptions: {},
      superFieldList: [],
      queryParam: {
