<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportXls('busi_om')" v-has="'BusiOmOrder:export'" >导出</a-button>
      ...
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
          <a-popconfirm title="确定推送吗?" @confirm="() => pushWMS(record)">
            <a v-if="record.query02 !=='已推送'">推送WMS</a>
          </a-popconfirm>
    ...
    <SampleMakingOutModal ref="modalForm" @ok="modalFormOk"/>
  </a-card>
</template>
...
<script>
  ...
  import SampleMakingOutModal from './modules/SampleMakingOutModal'
  import '@/assets/less/TableExpand.less'

  export default {
    name: "SampleMakingOut",
    mixins:[JeecgListMixin],
    components: {
      SampleMakingOutModal
    },
    data () {
      return {
        description: '出样管理页面',
    ...
    methods: {
      searchReset() {
        this.queryParam = {query01: "CYGL"}
        this.loadData()
      },
      pushWMS(e) {
        getAction('/jeeerp/busiOm/sampleOmPushWms',{ids: e.id}).then(res => {
          if(res.success) {
