  ...
  <a-card :bordered="false">
        ...
        <a-row :gutter="48">
          ...
          <a-col :md="!advanced && 8 || 24" :sm="24">
            <span class="table-page-search-submitButtons" :style="advanced && { float: 'right', overflow: 'hidden' } || {} ">
              ...
              <a-button style="margin-left: 8px">{{$t('重置')}}</a-button>
    ...
    <div class="table-operator">
      ...
      <a-dropdown v-if="selectedRowKeys.length > 0">
        <a-menu slot="overlay">
          <a-menu-item key="1"><a-icon type="delete" />{{$t('删除')}}</a-menu-item>
          <!-- lock | unlock -->
      ...
      <template slot="action" slot-scope="text, record, index">
        <div class="editable-row-operations">
          ...
          <span v-else>
            ...
            <a-divider type="vertical" />
            <a class="delete" @click="() => del(record)">{{$t('删除')}}</a>
          </span>
        </div>
      </template>
    </s-table>
    <j-modal title="新建规则" destroyOnClose :visible="visibleCreateModal" @ok="handleCreateModalOk" @cancel="handleCreateModalCancel">
      <!---->
