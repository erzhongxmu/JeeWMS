    ...
    <a-row :gutter="18">
      ...
      <a-col :span="8">
        <a-card :title="'已选' + name" :bordered="false" :head-style="{padding:0}" :body-style="{padding:0}">
          <a-table class="j-table-force-nowrap" size="middle" :rowKey="rowKey" bordered v-bind="selectedTable">
              <span slot="action" slot-scope="text, record, index">
                <a @click="handleDeleteSelected(record, index)">{{$t('删除')}}</a>
              </span>
          </a-table>
      ...
      return {
        ...
        selectedTable: {
          ...
          columns: [
            ...
            { title: this.$t('操作'), dataIndex: 'action', align: 'left', width: 60, scopedSlots: { customRender: 'action' }, }
          ],
          dataSource: [],
        },
