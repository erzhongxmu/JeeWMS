    ...
    <a-row type="flex">
      ...
      <a-col>
        ...
        <div v-if="actionButton" class="action-button">
          <a-button type="primary" icon="plus" @click="handleClickAdd" :disabled="disabled">{{$t('添加')}}</a-button>
          <span class="gap"></span>
          <template v-if="selectedRowIds.length>0">
            <a-popconfirm
              :title="`确定要删除这 ${selectedRowIds.length} 项吗?`"
              @confirm="handleConfirmDelete">
              <a-button type="primary" icon="minus" :disabled="disabled">{{$t('删除')}}</a-button>
              <span class="gap"></span>
            </a-popconfirm>
                ...
                <div
                  ...
                  <template v-for="(id,i) in [`${col.key}${row.id}`]">
                    ...
                    <template v-else-if="col.type === formTypes.popup">
                      <a-tooltip  :title="getPopupValue(id)" trigger="hover" placement="topLeft" >
                        <j-popup
                          v-if="isEditRow(row, col)"
                          :id="id"
                          :key="i"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      elemValueChange(type, rowSource, columnSource, value,others) {
        ...
        this.$emit('valueChange', { type, row, column, value, target: this ,others})
      },
      ...
      popupCallback(value, others, id, row, column, index) {
        // 存储输入的值
        console.log(value, others, id, row, column, index);
        ...
        this.validateOneInput(value, row, column, this.notPassedIds, true, 'change')
        // 触发valueChange 事件
        this.elemValueChange('input', row, column, value,others)
      },
      /** popup输入框回显 */
      getPopupValue(id) {
