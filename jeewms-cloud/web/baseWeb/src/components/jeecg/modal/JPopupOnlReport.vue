<template>
  <j-modal
    ...
    <div class="ant-alert ant-alert-info" style="margin-bottom: 16px;">
      ...
      {{$t('已选择')}}&nbsp;<a style="font-weight: 600">{{ table.selectedRowKeys.length }}</a>{{$t('项目')}}&nbsp;&nbsp;
  ...
  export default {
    ...
    methods:{
        ...
        httpGroupRequest(() => getAction(url), groupIdKey).then(res => {
          if(res.success){
            ...
            for(let a=0;a<currColumns.length;a++){
              currColumns[a].title = this.$t(currColumns[a].title)
      ...
      loadData(arg) {
        ...
        console.log(229,params,'-----------------')

        let arr = []
        this.queryInfo.map((item,index)=>{
          arr.push(item.field)
        })
        arr.map((item,index) =>{
