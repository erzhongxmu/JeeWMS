<template>
  <j-modal
      ...
      <a-form layout="inline" @keyup.enter.native="searchByquery">
        <a-row :gutter="24" v-if="showSearchFlag">
          <template v-for="(item,index) in queryInfo">
            <template v-if=" item.hidden==='1' && item.field != 'cus_code'">
            ...
            <template v-else-if="item.field != 'cus_code'">
          ...
          <a-col :md="8" :sm="8">
            <span style="float: left;overflow: hidden;" class="table-page-search-submitButtons">
              <a-button type="primary" @click="searchByquery" icon="search">{{$t('查询')}}</a-button>
    ...
    <div class="ant-alert ant-alert-info" style="margin-bottom: 16px;">
      ...
      已选择&nbsp;<a style="font-weight: 600">{{ table.selectedRowKeys.length }}</a>{{$t('项目')}}&nbsp;&nbsp;
...
<script>
  ...
  export default {
    ...
    props: ['multi', 'code', 'groupId','popupParam'],
    components:{
    },
    data(){
      return {
    ...
    watch: {
      popupParam: {
        ...
        handler: function(val) {
          if(val.type == '') {
            this.queryParam['cus_code'] = val.param
          } else if(val.type == 'bs') {
            this.queryParam['ku_wei_bian_ma'] = val.param
          }
          if(val) {
            this.loadColumnsInfo()
          }
