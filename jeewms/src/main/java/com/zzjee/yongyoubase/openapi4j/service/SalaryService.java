...
/**
 * Copyright(c) 2015-2015 by yonyouup. All Rights Reserved 工资记录
 * 
 * @author yanwuyang
...
public class SalaryService extends BaseService {
    ...
    final static Logger logger = LogManager.getLogger(SalaryService.class);

    public SalaryService() {
        this.access_token = TokenManager.getToKenId();
    }

    public SalaryService(String token) {
    ...
    public JSONObject get(String id, String to_account) throws OpenAPIException {
        ...
        try {
            ...
            paramMap.put("id", id);
            String url = this.createURL("salary/get", paramMap);
            logger.debug(url);
    ...
    public JSONObject batchGet(Map<String, String> paramMap) throws OpenAPIException {
        JSONObject record;
        try {
            String url = this.createURL("salary/batch_get", paramMap);
            logger.debug(url);
            record = JSONObject.parseObject(HttpUtil.get(url));
        } catch (Exception e) {
            throw new OpenAPIException(e.getMessage(), e);
