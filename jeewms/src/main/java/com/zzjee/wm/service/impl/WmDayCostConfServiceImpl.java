...
public class WmDayCostConfServiceImpl extends CommonServiceImpl implements WmDayCostConfServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(WmDayCostConfEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(WmDayCostConfEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	...
	private void doDelBus(WmDayCostConfEntity t) throws Exception{
 	...
 	private Map<String,Object> populationMap(WmDayCostConfEntity t){
		...
		map.put("cost_date", t.getCostDate());
		map.put("cost_sf", t.getCostSf());
		return map;
	}
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,WmDayCostConfEntity t){
 		...
 		sql  = sql.replace("#{cost_date}",String.valueOf(t.getCostDate()));
 		sql  = sql.replace("#{cost_sf}",String.valueOf(t.getCostSf()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("wm_day_cost_conf",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
