...
//cez
		...
		if("ldap".equals(ResourceUtil.getConfigByName("user.login.type"))){
			boolean ldapsu=true;
			try{
				if(!LdapUtil.ldapAuth(user.getUserName(), user.getPassword())){
					ldapsu=false;
				}
			}catch (Exception e){
...
//		List<TSUser> users = queryObject.list();
		...
		if("ldap".equals(ResourceUtil.getConfigByName("user.login.type"))){
			boolean ldapsu=true;
			try{
				if(!LdapUtil.ldapAuth(username, inpassword)){
					ldapsu=false;
				}
			}catch (Exception e){
