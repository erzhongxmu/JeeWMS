...
//		}
    ...
    protected void getField(StringBuffer sb, int frozen) {
        ...
        for (DataGridColumn column : columnList) {
            i++;
            Boolean bol = (column.isFrozenColumn() && frozen == 0) || (!column.isFrozenColumn() && frozen == 1);
            if (bol) {
                String field;
                if (treegrid) {
                ...
                if (!treegrid) {
                    // 字段排序
                    Boolean bol1 = (column.isSortable()) && (field.indexOf("_") <= 0 && field != "opt");
                    if (bol1) {
                ...
                if (columnStyleList.size() > 0 && !"opt".equals(column.getField())) {
                    ...
                    for (ColumnValue columnValue : columnStyleList) {
                        if (columnValue.getName().equals(column.getField())) {
                            ...
                            Boolean bol2 = (value.length == 0 || StringUtils.isEmpty(value[0])) && text.length == 1;
                            if (bol2) {
                                if (text[0].indexOf("(") > -1) {
