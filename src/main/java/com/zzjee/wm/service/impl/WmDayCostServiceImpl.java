...
public class WmDayCostServiceImpl extends CommonServiceImpl implements WmDayCostServiceI {
	...
	private void doDelBus(WmDayCostEntity t) throws Exception{
 	...
 	private Map<String,Object> populationMap(WmDayCostEntity t){
		...
		map.put("day_cost_yj", t.getDayCostYj());
		map.put("day_cost_bhs", t.getDayCostBhs());
		map.put("day_cost_se", t.getDayCostSe());
		map.put("day_cost_hsj", t.getDayCostHsj());
		map.put("cost_ori", t.getCostOri());
		map.put("beizhu", t.getBeizhu());
		map.put("cost_sta", t.getCostSta());
		map.put("cost_sl", t.getCostSl());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,WmDayCostEntity t){
 		...
 		sql  = sql.replace("#{day_cost_yj}",String.valueOf(t.getDayCostYj()));
 		sql  = sql.replace("#{day_cost_bhs}",String.valueOf(t.getDayCostBhs()));
 		sql  = sql.replace("#{day_cost_se}",String.valueOf(t.getDayCostSe()));
 		sql  = sql.replace("#{day_cost_hsj}",String.valueOf(t.getDayCostHsj()));
 		sql  = sql.replace("#{cost_ori}",String.valueOf(t.getCostOri()));
 		sql  = sql.replace("#{beizhu}",String.valueOf(t.getBeizhu()));
 		sql  = sql.replace("#{cost_sta}",String.valueOf(t.getCostSta()));
 		sql  = sql.replace("#{cost_sl}",String.valueOf(t.getCostSl()));
 		sql  = sql.replace("#{cost_unit}",String.valueOf(t.getCostUnit()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("wm_day_cost",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
