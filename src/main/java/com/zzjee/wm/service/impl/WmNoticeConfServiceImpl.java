...
public class WmNoticeConfServiceImpl extends CommonServiceImpl implements WmNoticeConfServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(WmNoticeConfEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(WmNoticeConfEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	...
	private void doDelBus(WmNoticeConfEntity t) throws Exception{
 	...
 	private Map<String,Object> populationMap(WmNoticeConfEntity t){
		...
		map.put("wm_notice_id", t.getWmNoticeId());
		map.put("beizhu", t.getBeizhu());
		map.put("cus_code", t.getCusCode());
		map.put("hd_data", t.getHdData());
		return map;
	}
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,WmNoticeConfEntity t){
 		...
 		sql  = sql.replace("#{wm_notice_id}",String.valueOf(t.getWmNoticeId()));
 		sql  = sql.replace("#{beizhu}",String.valueOf(t.getBeizhu()));
 		sql  = sql.replace("#{cus_code}",String.valueOf(t.getCusCode()));
 		sql  = sql.replace("#{hd_data}",String.valueOf(t.getHdData()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("wm_notice_conf",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
