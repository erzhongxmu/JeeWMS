...
public class BaPlatformServiceImpl extends CommonServiceImpl implements BaPlatformServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(BaPlatformEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(BaPlatformEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	...
	private void doDelBus(BaPlatformEntity t) throws Exception{
 	...
 	private Map<String,Object> populationMap(BaPlatformEntity t){
		...
		map.put("platform_code", t.getPlatformCode());
		map.put("platform_name", t.getPlatformName());
		map.put("platform_del", t.getPlatformDel());
		return map;
	}
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,BaPlatformEntity t){
 		...
 		sql  = sql.replace("#{platform_del}",String.valueOf(t.getPlatformDel()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("ba_platform",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
