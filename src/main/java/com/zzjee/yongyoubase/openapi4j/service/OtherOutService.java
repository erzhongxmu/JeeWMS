...
public class OtherOutService extends BaseService {
    ...
    final static Logger logger = LogManager.getLogger(OtherOutService.class);

    public OtherOutService() {
        this.access_token = TokenManager.getToKenId();
    }

    public OtherOutService(String token) {
    ...
    public JSONObject get(String id, String to_account) throws OpenAPIException {
        ...
        try {
            ...
            paramMap.put("id", id);
            String url = this.createURL("otherout/get", paramMap);
            logger.debug(url);
    ...
    public JSONObject batchGet(Map<String, String> paramMap) throws OpenAPIException {
        JSONObject record;
        try {
            String url = this.createURL("otheroutlist/batch_get", paramMap);
            logger.debug(url);
            record = JSONObject.parseObject(HttpUtil.get(url));
        } catch (Exception e) {
            throw new OpenAPIException(e.getMessage(), e);
    ...
    public JSONObject add(String jsonBody, String to_account) throws OpenAPIException {
        ...
        try {
            ...
            String url = this.createURL("otherout/add", paramMap);
            logger.debug(url);
